...
<template>
  <div :class="['transition-colors pb-16', isRtl ? 'text-right' : 'text-left']" :dir="isRtl ? 'rtl' : 'ltr'">
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-20">
      ...
      <p class="mt-4 animate-pulse" style="color: var(--q-muted)">
        {{ t('qa.loading') }}
      </p>
    </div>
    ...
    <div v-else-if="qaData" class="px-4 mx-auto space-y-6 max-w-screen-2xl sm:px-6 lg:px-8">
      ...
      <div class="flex flex-wrap gap-3 mb-4 text-sm">
        ...
        <span class="qa-badge inline-flex items-center">
          <Atom class="w-3.5 h-3.5" :class="isRtl ? 'ml-1' : 'mr-1'" />{{ qaData.config.n_qubits }} {{ t('qa.qubits')
          }}, {{ t('qa.depth') }} {{ qaData.config.q_depth }}
        </span>
        <span class="qa-badge inline-flex items-center">
      ...
      <section class="qa-card">
        <div class="qa-card-header">
          ...
          <div>
            <h2 class="qa-card-title">
              {{ t('qa.exp2.title') }}
            </h2>
            <p class="qa-card-desc">
              {{ t('qa.exp2.desc') }}
            </p>
          </div>
        </div>
        <div class="flex flex-wrap items-center gap-4 mb-6">
          <div class="inline-flex items-center gap-2 px-5 py-3 rounded-xl"
          ...
          <Info v-tooltip.top="t('qa.exp2.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon"
            style="color: var(--q-muted)" />
        </div>
        <div class="h-72" role="img" :aria-label="t('qa.exp2.title')">
      ...
      <section v-if="noiseTypes.length" class="qa-card">
        ...
        <div class="h-72" :aria-label="t('qa.exp6.title')">
      ...
      <section class="qa-card">
        <div class="qa-card-header">
          ...
          <Info v-tooltip.top="t('qa.exp1.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
        </div>
        <div class="flex flex-col items-center gap-8 md:flex-row">
          <div class="flex justify-center flex-1">
            <svg width="220" height="140" viewBox="0 0 220 140">
              <path d="M 20 130 A 90 90 0 0 1 200 130" fill="none" stroke="var(--q-bar-border)" stroke-width="14"
                stroke-linecap="round" />
              <path :d="`M 20 130 A 90 90 0 0 1 ${halfGaugeX} ${halfGaugeY}`" fill="none" stroke="#22C55E"
                stroke-width="14" stroke-linecap="round" />
              <line x1="110" y1="130" :x2="halfNeedleX" :y2="halfNeedleY" stroke="var(--q-teal)" stroke-width="3"
                stroke-linecap="round" />
          ...
          <div class="flex-1 space-y-3">
            <div class="text-4xl" style="color: var(--q-text)">
              {{ primaryOrthogonality.toFixed(3) }}
            </div>
            <div class="text-sm" style="color: var(--q-muted)">
              {{ t('qa.exp1.score') }}
            </div>
            <div class="inline-block px-3 py-1 bg-[#22C55E]/10 text-[#22C55E] rounded-full text-sm">
              {{ t('qa.exp1.target') }}
            </div>
            <p class="text-sm" style="color: var(--q-muted)">
              {{ t('qa.exp1.explanation') }}
            </p>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <section v-if="ckaData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp13.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div v-for="d in ckaData" :key="d.model" class="qa-metric-card">
              ...
              <div class="text-3xl font-mono" style="color: var(--q-text)">
                {{ d.cka.toFixed(3) }}
              </div>
        ...
        <section class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp3.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div class="qa-metric-card">
              ...
              <div class="text-2xl" style="color: var(--q-text)">
                {{ reuploadData.with }}%
              </div>
            </div>
            <div class="qa-metric-card">
      ...
      <section class="qa-card">
        <div class="qa-card-header">
          ...
          <Info v-tooltip.top="t('qa.exp4.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
        </div>
        <div class="flex flex-col gap-8 lg:flex-row">
          <div class="flex flex-col items-center gap-2">
            <div class="relative flex items-center justify-center">
              <svg :width="circSize" :height="circSize" class="-rotate-90">
                <circle :cx="circSize / 2" :cy="circSize / 2" :r="circR" fill="none" stroke="var(--q-bar-border)"
                  stroke-width="10" />
                <circle :cx="circSize / 2" :cy="circSize / 2" :r="circR" fill="none" stroke="var(--q-teal)"
                  stroke-width="10" :stroke-dasharray="circCircumference" :stroke-dashoffset="circOffset"
                  stroke-linecap="round" />
              </svg>
              <div class="absolute flex flex-col items-center justify-center"
                :style="{ width: circSize + 'px', height: circSize + 'px' }">
          ...
          <div class="flex-1 h-64" role="img" :aria-label="t('qa.exp4.title')">
      ...
      <section class="qa-card">
        <div class="qa-card-header">
          ...
          <Info v-tooltip.top="t('qa.exp5.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
        </div>
        <Card class="q-glass qa-inner-card">
          <template #content>
            <DataTable :value="gradientVarianceRows" :row-class="rowClass"
              :class="['text-sm p-datatable-sm', isRtl ? 'qa-table-rtl' : '']">
              <Column field="model" :header="t('qa.model')">
                <template #body="{ data }">
                  <span style="color: var(--q-text)" :class="{ 'font-medium': data.highlight }">{{ data.model }}</span>
                  <Tag v-if="data.highlight" value="Baseline"
                    class="ml-2 bg-[#6366F1]/10 text-[#6366F1] !text-xs !py-0.5" />
                </template>
              </Column>
              <Column field="layer" :header="t('qa.exp5.target')">
                <template #body="{ data }">
                  <span class="font-mono text-xs" style="color: var(--q-muted)">{{ data.layer
                    }}</span>
                </template>
              </Column>
              <Column field="mean_var" :header="t('qa.exp5.meanVar')">
                <template #body="{ data }">
                  <span class="font-mono" style="color: var(--q-text)">{{
                    formatExp(data.mean_var) }}</span>
                </template>
              </Column>
              <Column field="abs_mean" :header="t('qa.exp5.absMean')">
                <template #body="{ data }">
                  <span class="font-mono" style="color: var(--q-text)">{{
                    formatExp(data.abs_mean) }}</span>
                </template>
              </Column>
              <Column field="batches" :header="t('qa.exp5.batches')" />
            </DataTable>
          </template>
        </Card>
        <div class="qa-note-green mt-4">
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section v-if="expressibilityData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp7.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <div class="grid grid-cols-1 gap-4">
            <div v-for="d in expressibilityData" :key="d.model" class="qa-metric-card">
              <div class="text-sm font-medium" style="color: var(--q-text)">
                {{ d.model }}
              </div>
              <div class="text-3xl font-mono mt-1" style="color: var(--q-teal)">
                {{ d.kl.toFixed(4) }}
              </div>
              <div class="text-xs mt-1" style="color: var(--q-muted)">
                {{ t('qa.exp7.klDiv') }}
              </div>
              <div class="text-xs mt-2" style="color: var(--q-muted)">
                {{ t('qa.exp7.ref') }}: {{ d.ref }}
              </div>
            </div>
          </div>
        ...
        <section v-if="geoData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp9.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <div class="grid grid-cols-1 gap-4">
            <div v-for="d in geoData" :key="d.model" class="qa-metric-card">
              ...
              <div class="mt-2">
                <Tag :value="d.advantage ? t('qa.confirmed') : t('qa.notConfirmed')"
                  :class="d.advantage ? 'bg-[#22C55E]/10 text-[#22C55E]' : 'bg-red-500/10 text-red-400'"
                  class="!text-xs" />
              </div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section v-if="ktaData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp8.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <Card class="q-glass qa-inner-card">
            <template #content>
                ...
                <Column :header="t('qa.exp8.quantum')">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-teal)">{{
                      data.quantum.toFixed(4) }}</span>
                  </template>
                </Column>
                <Column :header="t('qa.exp8.classical')">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-text)">{{
                      data.classical.toFixed(4) }}</span>
                  </template>
                </Column>
                <Column :header="t('qa.exp8.diff')">
                  <template #body="{ data }">
                    <span class="font-mono" :class="data.diff > 0 ? 'text-[#22C55E]' : 'text-[#F59E0B]'">{{ data.diff >
                      0 ? '+' : '' }}{{ data.diff.toFixed(4) }}</span>
                  </template>
                </Column>
              </DataTable>
            </template>
        ...
        <section v-if="fisherData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp10.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <Card class="q-glass qa-inner-card">
            <template #content>
              <DataTable :value="fisherData" :class="['text-sm p-datatable-sm', isRtl ? 'qa-table-rtl' : '']">
                <Column field="model" :header="t('qa.model')">
                  <template #body="{ data }">
                    <span style="color: var(--q-text)">{{ data.model }}</span>
                    <Tag v-if="data.model.includes('CNN')" value="Baseline"
                      class="ml-2 bg-[#6366F1]/10 text-[#6366F1] !text-xs !py-0.5" />
                  </template>
                </Column>
                <Column :header="t('qa.exp10.params')">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-text)">{{ data.nParams
                      }}</span>
                  </template>
                </Column>
                <Column :header="t('qa.exp10.dEff1000')">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-teal)">{{
                      data.dEff?.['1000']?.toFixed(2) || 'N/A' }}</span>
                  </template>
                </Column>
                <Column :header="t('qa.exp10.dEffPerParam')">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-text)">{{
                      data.dEffPerParam?.['1000']?.toFixed(6) || 'N/A' }}</span>
                  </template>
                </Column>
              </DataTable>
            </template>
      ...
      <section v-if="effectiveRankData.length" class="qa-card">
        <div class="qa-card-header">
          ...
          <Info v-tooltip.top="t('qa.exp11.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
        </div>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div v-for="d in effectiveRankData" :key="d.model" class="qa-metric-card space-y-3 text-start">
            ...
            <div class="w-full rounded-full h-2 overflow-hidden" style="background: var(--q-bar-border)">
              <div class="h-full rounded-full bg-[#6366F1]" :style="{ width: pct(d.zUtil) + '%' }" />
            </div>
            <div class="flex justify-between text-sm">
            ...
            <div class="w-full rounded-full h-2 overflow-hidden" style="background: var(--q-bar-border)">
              <div class="h-full rounded-full" style="background: var(--q-teal)"
                :style="{ width: pct(d.qUtil) + '%' }" />
            </div>
          </div>
        </div>
        <div class="qa-note mt-4">
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section v-if="intrinsicDimData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp12.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <div class="grid grid-cols-1 gap-4">
            <div v-for="d in intrinsicDimData" :key="d.model" class="qa-metric-card">
              ...
              <div class="flex gap-6 mt-2 justify-center">
                <div>
                  <div class="text-xs" style="color: var(--q-muted)">
                    {{ t('qa.exp12.classical') }}
                  </div>
                  <div class="text-xl font-mono" style="color: var(--q-text)">
                    {{ d.dimZ.toFixed(2) }}
                  </div>
                </div>
                <div>
                  ...
                  <div class="text-xl font-mono" style="color: var(--q-teal)">
                    {{ d.dimQ.toFixed(2) }}
                  </div>
                </div>
              </div>
        ...
        <section v-if="separabilityData.length" class="qa-card">
          <div class="qa-card-header">
            ...
            <Info v-tooltip.top="t('qa.exp14.tooltip')" class="w-4 h-4 flex-shrink-0 cursor-help qa-info-icon" />
          </div>
          <Card class="q-glass qa-inner-card">
            <template #content>
              <DataTable :value="separabilityData" :class="['text-sm p-datatable-sm', isRtl ? 'qa-table-rtl' : '']">
                ...
                <Column header="J(z)">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-text)">{{
                      data.fisherZ.toFixed(2) }}</span>
                  </template>
                </Column>
                <Column header="J(z_proj)">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-text)">{{
                      data.fisherZProj.toFixed(2) }}</span>
                  </template>
                </Column>
                <Column header="J(q_emb)">
                  <template #body="{ data }">
                    <span class="font-mono" style="color: var(--q-teal)">{{
                      data.fisherQ.toFixed(2) }}</span>
                  </template>
                </Column>
                <Column :header="t('qa.exp14.advantage')">
                  <template #body="{ data }">
                    <Tag :value="data.advantage ? '✓' : '✗'"
                      :class="data.advantage ? 'bg-[#22C55E]/10 text-[#22C55E]' : 'bg-[#F59E0B]/10 text-[#F59E0B]'"
                      class="!text-xs" />
                  </template>
                </Column>
              </DataTable>
      ...
      <section v-if="methodologyNotes.length" class="qa-card">
        <div class="qa-card-header">
          ...
          <h2 class="qa-card-title">
            {{ t('qa.methodology.title') }}
          </h2>
        </div>
        <Accordion v-model:value="activePanels" multiple>
          <AccordionPanel v-for="(note, i) in methodologyNotes" :key="i" :value="i">

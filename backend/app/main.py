...
async def classify_image(file: UploadFile = File(...)):
    ...
    # contents is raw bytes — pass to your model here
...
if os.path.exists(frontend_path):
    ...
    async def serve_frontend(full_path: str, request: Request):
        ...
        if full_path.startswith("api/"):
            ...
            return JSONResponse(
                status_code=404, content={"message": "API route not found"}
            )
    ...
    error_msg = (
        f"Frontend dist not found at: {frontend_path}. Static serving will not work."
    )

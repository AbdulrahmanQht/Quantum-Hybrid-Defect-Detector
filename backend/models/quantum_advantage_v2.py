...
What this computes:
    ...
    6. Noise Robustness Ablation  — runs full / classical_only / quantum_only forward
                                    passes under 5 noise types at multiple intensity
    ...
    14. Linear Probe              — Trains a logistic regression on three frozen feature
                                    sets: z (512-dim), q_emb (128-dim), and their
                                    concatenation. Accuracy gaps quantify each branch's
                                    standalone discriminative power. Reviewer-trusted
                                    because it does not touch the trained classifier head.

    15. Class Separability        — Multiclass Fisher criterion J = tr(S_W^{-1} S_B)
...
CONFIG = {
    ...
    "grad_var_batches":   20,
    "entropy_samples":    128,
    "feature_samples":    256,           # feature vectors collected per model (Exp 8–15)
...
OUTPUT_PATH = os.path.join(BACKEND_ROOT, "data", "QA", "V2", "quantum_advantage_results.json")
OUTPUT_DIR  = os.path.join(BACKEND_ROOT, "data", "QA", "V2")
...
def _collect_all_features(
    ...
    """
    Single-pass per-model feature collection for Experiments 8–15.
    ...
    for model_key, model in models.items():
        ...
        for images, labels in test_loader:
            ...
            if isinstance(model, HybridQnnCPU):
                ...
                with torch.amp.autocast(device_type=device.type, enabled=False):
                    q_raw = model.q_layer(q_in_scaled.float())
...
def run_kernel_experiments(
    ...
    for label, model in models.items():
        ...
        n = min(kernel_samples, len(feats["q_in"]))
        q_in   = feats["q_in"][:n]      # (n, n_qubits)
        labels = feats["labels"][:n]    # (n,)

        #  Quantum kernel 
        logger.info(f"  [{label}] Computing quantum kernel ({n}×{n}) …")
        weights = model.q_layer.weights.detach().cpu().float()
        K_Q = _compute_quantum_kernel(circuit, weights, q_in, n_qubits)

        #  Classical RBF kernel on same 6-dim input 
        K_C = _rbf_kernel(q_in)
        ...
        geom_results[label] = {
            ...
            "interpretation": (
                ...
                f"g = {g:.4f} — {'quantum advantage confirmed' if g > 1 else 'no geometric advantage; frame as practical hybrid advantage'}."
            ),
        }
...
def run_linear_probe(
    ...
    """
    Trains a logistic regression on three frozen feature sets:
        backbone (z, 512-dim), quantum embedding (q_emb, 128-dim), and concat.

    The accuracy gaps quantify each branch's standalone discriminative power.
    Does not touch the trained classifier head — reviewers trust this ablation.
    Uses an 80/20 train/test split on the collected test-set features.
    """
    if not SKLEARN_AVAILABLE:
        logger.warning("sklearn not available — skipping Experiment 14 (Linear Probe).")
        return {k: {"error": "sklearn not installed"} for k in collected_features}
    ...
    for label, feats in collected_features.items():
        ...
        q_emb = feats["q_emb"]
        y     = feats["labels"]
        ZQ    = np.concatenate([z, q_emb], axis=1)

        def probe(X: np.ndarray, name: str) -> float:
            ...
            clf.fit(X_tr, y_tr)
            acc = round(100.0 * float(clf.score(X_te, y_te)), 4)
            logger.info(f"    Linear probe [{label}/{name}]: {acc:.2f}%")
            return acc

        backbone_acc = probe(z,    "backbone")
        quantum_acc  = probe(q_emb, "quantum")
        concat_acc   = probe(ZQ,   "concat")

        results[label] = {
            "backbone_only_%":          backbone_acc,
            "quantum_only_%":           quantum_acc,
            "concat_%":                 concat_acc,
            "quantum_standalone_gain":  round(quantum_acc  - backbone_acc, 4),
            "concat_gain_over_backbone": round(concat_acc  - backbone_acc, 4),
            "n_samples":                int(len(y)),
            "interpretation": (
                "Linear probe accuracy on frozen features. "
                "quantum_standalone_gain > 0 means q_emb alone is more discriminative "
                "than z alone. concat_gain_over_backbone > 0 confirms the branches "
...
#  Experiment 15 — Class Separability 
...
def run_class_separability(
    ...
    for label, feats in collected_features.items():
        ...
        logger.info(
            ...
            f"({'quantum better' if J_q > J_z else 'classical better'})"
        )

    return results


#  CSV Exports — Original 
...
def export_linear_probe_csv(results: dict, output_dir: str) -> None:
    path = os.path.join(output_dir, "qa_linear_probe.csv")
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "model", "backbone_only_%", "quantum_only_%", "concat_%",
            "quantum_standalone_gain_%", "concat_gain_over_backbone_%",
        ])
        for model_key, r in results.items():
            writer.writerow([
                model_key,
                _na(r.get("backbone_only_%")),
                _na(r.get("quantum_only_%")),
                _na(r.get("concat_%")),
                _na(r.get("quantum_standalone_gain")),
                _na(r.get("concat_gain_over_backbone")),
            ])
    logger.info(f"CSV saved: {path}")
...
def run_quantum_advantage() -> dict:
    ...
    noise_ablation_results = run_noise_ablation(
        qnn_models, test_loader, device, noise_levels=QA_NOISE_LEVELS,
    )

    #  Feature collection (shared across Experiments 8–15) 

    logger.info("\n=== Feature Collection (Experiments 8–15) ===")
    logger.info(f"  Collecting {cfg['feature_samples']} vectors per model …")
    collected_features = _collect_all_features(
        qnn_models, test_loader, device, n_samples=cfg["feature_samples"]
    )
    ...
    logger.info(
        f"  Quantum kernel: {cfg['kernel_samples']}×{cfg['kernel_samples']} matrix. "
        "Computing statevectors on default.qubit …"
    )
    ...
    #  Experiment 10: FIM + Effective Dimension 
    ...
    logger.info("\n=== Experiment 14: Linear Probe (frozen features) ===")
    linear_probe_results = run_linear_probe(qnn_models, collected_features)

    #  Experiment 15: Class Separability 

    logger.info("\n=== Experiment 15: Class Separability (Fisher Criterion) ===")
    ...
    output = {
        ...
        "experiment_14_linear_probe":             linear_probe_results,
        "experiment_15_class_separability":       separability_results,
    }
    ...
    export_linear_probe_csv(linear_probe_results, OUTPUT_DIR)
    ...
    for model_key in qnn_models:
        ...
        print(f"    [2]  Quantum branch gain:          {abl.get('quantum_gain_%', 'N/A'):+.2f}%")

        reu = reupload_results.get(model_key, {})
        print(f"    [3]  Re-upload contribution:       {reu.get('reupload_contribution_%', 'N/A'):+.2f}%")
        ...
        lp = linear_probe_results.get(model_key, {})
        if lp.get("backbone_only_%") is not None:
            print(f"    [14] Linear probe backbone/q/cat: "
                  f"{lp['backbone_only_%']:.1f}% / "
                  f"{lp['quantum_only_%']:.1f}% / "
                  f"{lp['concat_%']:.1f}%")
        ...
        if J_z is not None:
            ...
            print(f"    [15] Fisher criterion z / q_emb:  {J_z} / {J_q}  ({tag_sep})")

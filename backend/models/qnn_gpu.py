"""
HybridQnnGPU — Noise-robust hybrid quantum-classical model for industrial defect detection.
...
class HybridQnnGPU(nn.Module):
    ...
    def fit(
        ...
        *,
        ...
        checkpoint_path: str = "models/qnn_gpu_gpu.pth",
        resume_checkpoint_path: str = "models/qnn_gpu_resume_gpu.pth",
        use_class_weights: bool = True,
        skip_prompt: bool = True,
    ) -> None:
        self.to(device)

        start_msg = "\n" + "=" * 45 + " HYBRID QNN-GPU BEGINS TRAINING\n" + "=" * 45
        print(start_msg)
        self.logger.info("HYBRID QNN-GPU BEGINS TRAINING")
        ...
        # Build optimizer / scheduler / scaler BEFORE loading resume checkpoint
        # so their state_dicts can be restored immediately after construction.
        q_params = (
            list(self.q_layer.parameters())
            + list(self.quantum_selector.parameters())
            + [self.angle_scales]
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if va_acc > best_acc:
                ...
                best_shadow = copy.deepcopy(self._quantum_shadow)
                live_backup = self._apply_ema()
